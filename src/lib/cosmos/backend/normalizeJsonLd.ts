/**
 * Solución de normalización compatible con tipos para jsonld sin dependencia de jsonld-spec
 */
 ...
 */
export function getDocumentLoader(schemaMap: SchemaMap = {}):
  (url: string, callback?: (err: Error, remoteDoc: RemoteDocument) => void) => Promise<RemoteDocument> {
  ...
  return async function (
    ...
    try {
      ...
      if (schemaMap[url]) {
        console.log(`[getDocumentLoader] Found context in schemaMap for: ${url}`);
        ...
        console.log(`[getDocumentLoader] Fetching document from network: ${url}`);
        ...
        console.log(`[getDocumentLoader] Successfully fetched external context for: ${url}`);
      ...
      if (callback) {
        console.log(`[getDocumentLoader] Calling callback for successful load: ${url}`);
      ...
      console.error(`[getDocumentLoader] Failed to load context from: ${url}`);
 ...
 */
export async function normalizeJsonLd(document: any, schemaMap: SchemaMap = {}): Promise<string> {
  const options = {
